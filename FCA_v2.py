...
SourceIDField = "ORIG_FID"                      # unique ID for each origin (e.g., municipality code, or generated ID for a point in grid), don't change
JoinIDField = "OriginID_Point"                  # new field in origins layer to join with OD matrix, equals to OBJECTID!
PopField = ""                                   # population field in origins layer
destinationIdField = "OBJECTID"                 # don't change
supplyField = ""                            # don't change unless neccessary, needs to be created manually in the destination layer
# DIDField = "DID"                                
...
def compute_accessibility_2sfca(df_od, df_rj_2sfca):
    ...
    df_merged["SPAI_term"] = df_merged["Rj"] * df_merged["f_dij"]
    df_spai_2sfca = (df_merged.groupby(OD_SourceIDField)["SPAI_term"].sum().reset_index(name="SPAI_2sfca"))
...
def compute_accessibility_e2sfca(df_od, df_rj_e2sfca):
    ...
    # Compute SPA_i = SUM( R_j * f(d_ij) ) for each origin
    df_merged["SPAI_term"] = df_merged["Rj"] * df_merged["f_dij"]
    df_spai_e2sfca = (df_merged.groupby(OD_SourceIDField)["SPAI_term"].sum().reset_index(name="SPAI_e2sfca"))
...
df_spai_m2sfca = compute_accessibility_m2sfca(df_od, df_rij)
